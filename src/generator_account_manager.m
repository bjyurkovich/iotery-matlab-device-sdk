api = webread(['https://raw.githubusercontent.com/bjyurkovich/iotery-node-server-sdk/master/spec/api.json']);
api = jsondecode(api);
routes = api.routes;

template = fileread('template_account_manager_class.m');
...
for i=1:length(routes)
    ...
    docMethods{end} = strrep(docMethods{end}, 'varargin', '');
    docMethods{end} = strrep(docMethods{end}, ',  ', '');
...
%     
    ...
    f = fopen('IoteryAccountManager.m', 'w');
    fwrite(f, out);
    fclose(f);
